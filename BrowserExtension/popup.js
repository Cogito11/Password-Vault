...
async function fillEntry(id) { await send({ type: 'fillCredential', id, tabId: activeTab.id }); message('Login filled.'); }
...
$('#lockButton').onclick = async () => { await send({ type: 'lockVault' }); await loadBooks(); };
$('#addToggle').onclick = () => { $('#entryForm').hidden = !$('#entryForm').hidden; };
$('#generate').onclick = async () => { $('#entryPassword').value = (await send({ type: 'generatePassword', length: 20 })).password; };
