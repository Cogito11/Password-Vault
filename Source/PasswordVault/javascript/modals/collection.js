...
function addAttrRow(key, val, shouldFocus) {
	...
	row.querySelector('.attr-row-del').addEventListener('click', function () { attrRows.removeChild(wrap); });

	wireAttrGenerator(wrap);

	attrRows.appendChild(wrap);
...
entryNameInput.addEventListener('keydown', function (e) {
	if (e.key === 'Enter') 
	{
		e.preventDefault(); addEntryBtn.click(); 
	}
});
...
collNameInput.addEventListener('keydown', function (e) {
	if (e.key === 'Enter') 
	{ 
		e.preventDefault(); saveCollBtn.click(); 
	}
});
